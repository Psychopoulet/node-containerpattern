...
// consts
const VALID_SKELETONS = ["array", "boolean", "color", "email", "float", "ipv4", "ipv6", "integer", "object", "string", "url", "serial"];
const MIN_MAX_SKELETONS = ["array", "color", "email", "float", "ipv4", "ipv6", "integer", "string", "url", "serial"];
const REGEX_SKELETONS = ["color", "email", "ipv4", "ipv6", "string", "url", "serial"];
// module
class NodeContainerPattern extends Map {
    ...
    _ensureData(key, value) {
        ...
        // check skeleton
            ...
            else if ((0, validators_1.inArray)(["color", "email", "ipv4", "ipv6", "url", "serial"], skeleton)) {
    ...
    min(_key, min) {
        ...
        else if (["array", "color", "email", "integer", "ipv4", "ipv6", "string", "url", "serial"].includes(this.skeletons[key]) && !(0, validators_1.isInteger)(min)) {
            throw new TypeError("The \"" + key + "\" data has an invalid \"min\" attribute");
        }
        else {
            this.mins[key] = min;
    ...
    max(_key, max) {
        ...
        else if (["array", "color", "email", "integer", "ipv4", "ipv6", "string", "url", "serial"].includes(this.skeletons[key]) && !(0, validators_1.isInteger)(max)) {
            throw new TypeError("The \"" + key + "\" data has an invalid \"max\" attribute");
        }
        else {
            this.maxs[key] = max;
