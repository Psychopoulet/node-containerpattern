...
class NodeContainerPattern extends Map {
    ...
    _ensureData(key, value) {
        ...
        // check skeleton
            ...
            else if ((0, validators_1.inArray)(DATA_SPECIFIC, skeleton)) {
                return (0, ensureDataSpecific_1.default)(key, skeleton, value);
            }
            else if ((0, validators_1.inArray)(["array", "string"], skeleton)) {
                const result = "array" === skeleton
                    ? (0, ensureDataArray_1.default)(key, skeleton, value)
                    : (0, ensureDataBasic_1.default)(key, skeleton, value);
    ...
    _createBaseObject(parentKey, _parentValue, keys, value) {
        ...
        if (keys.length) {
            ...
            parentValue[key] = this._createBaseObject(parentKey + this.recursionSeparator + key, "undefined" === typeof parentValue[key]
                ? defaultValue
                : parentValue[key], keys, value);
        }
        else {
    ...
    _extractDocumentation(previousKeys, object) {
        ...
        toExtract.forEach(({ value, key }) => {
            ...
            const type = (0, validators_1.isNotEmptyString)(this.skeletons[fullKey])
                ? this.skeletons[fullKey]
                : (0, getTypeValue_1.default)(value);
            const documentation = {
                "fullkey": fullKey,
                type
            };
            ...
            if ("array" === type) {
                documentation.content = (0, validators_1.isEmptyArray)(value)
                    ? {}
                    : this._extractDocumentation(fullKey, value);
            }
            else if ("object" === type) {
                documentation.content = (0, validators_1.isEmptyPlainObject)(value)
                    ? {}
                    : this._extractDocumentation(fullKey, value);
            }
    ...
    min(_key, min) {
        ...
        if (!(0, validators_1.isDefined)(min)) {
            ...
            "url",
            "serial"
        ].includes(this.skeletons[key]) && !(0, validators_1.isInteger)(min)) {
            throw new TypeError("The \"" + key + "\" data has an invalid \"min\" attribute");
        }
        else {
            this.mins[key] = min;
    ...
    max(_key, max) {
        ...
        if (!(0, validators_1.isDefined)(max)) {
            ...
            "url",
            "serial"
        ].includes(this.skeletons[key]) && !(0, validators_1.isInteger)(max)) {
            throw new TypeError("The \"" + key + "\" data has an invalid \"max\" attribute");
        }
        else {
            this.maxs[key] = max;
